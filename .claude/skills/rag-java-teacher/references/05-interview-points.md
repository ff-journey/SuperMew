...
## Q4: 层级分块和Auto-merge解决什么问题？

**Answer**:
这是一个精度和上下文的trade-off问题：

- **短chunk（L3, 300词）**：检索精度高，每个chunk主题集中
- **长chunk（L1, 1200词）**：上下文完整，但检索时可能命中不那么精准

**Auto-merge的思路**：只存L3 leaf chunk到Milvus做检索，但如果同一个文档段落里有≥2个兄弟L3 chunk同时被检索命中，说明这段内容整体都相关，就把它们合并为L2父chunk，给LLM更完整的上下文。

这就是SuperMew `rag_utils.py` 里 `_merge_to_parent_level()` 做的事：L3→L2→L1递归合并。

关键设计：L1/L2不写入Milvus（避免冗余向量，降低检索噪声），只存在 `parent_chunks.json`。
